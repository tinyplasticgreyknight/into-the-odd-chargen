...
def load_data(datafile):
    global MEMOIZED_DATA
    if datafile not in MEMOIZED_DATA:
        with open(datafile+".txt", 'r') as f:
            MEMOIZED_DATA[datafile] = yaml.load(f)
    return MEMOIZED_DATA[datafile]

def data_table(datafile, section):
    filedata = load_data(datafile)
    return filedata[section]

def datum(datafile, section, key):
    return data_table(datafile, section)[key]
